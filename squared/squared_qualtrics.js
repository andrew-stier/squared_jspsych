...
// Qualtrics integration entry point. The Qualtrics question's addOnload handler
// loads jsPsych + plugins + this file, then calls initExp().
var asset_root = "https://andrew-stier.github.io/squared_jspsych/squared/";
	...
	on_finish: function() {
		...
		tasks.forEach(function(t) {
			...
			[1, 2, 3, 4].forEach(function(c) {
				Qualtrics.SurveyEngine.setEmbeddedData(t + '_score_' + c, last['score_' + c]);
				Qualtrics.SurveyEngine.setEmbeddedData(t + '_meanrt_' + c, Math.round(last['meanrt_' + c]));
			});
		});

		// Tear down the injected display container so it doesn't sit on the next page.
		if (typeof jQuery !== 'undefined') {
			jQuery('#display_stage').remove();
			jQuery('#display_stage_background').remove();
		}

		// Advance the Qualtrics survey.
...
var online = 1; // Always 1 in Qualtrics build; suppresses localSave('csv') in conclusion_* trials
var subject = (typeof window !== 'undefined' && window.QUALTRICS_PID) ? window.QUALTRICS_PID : ('anon_' + Date.now());
...
const ar = asset_root + "ar.PNG";
const al = asset_root + "al.PNG";
const mr_fl = asset_root + "mr_fl.PNG";
const ml_fr = asset_root + "ml_fr.PNG";
...
const rarr = asset_root + "rarr.PNG";
const larr = asset_root + "larr.PNG";
...
// Qualtrics build: skip get_participant_id (PID supplied via window.QUALTRICS_PID),
// skip get_location (online forced to 1), and skip enter/exit_fullscreen (often
// blocked inside Qualtrics iframes).
timeline.push(preload, welcome, stroop_task, flanker_task, simon_task, conclusion);
